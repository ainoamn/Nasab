...
| البند | الحالة |
|--------|--------|
| دخول المشرف عبر Neon SQL (HTTP) مباشرة | ✅ |
| فحص اتصال حقيقي: `GET /api/diag?db=1` → `dbOk`/`dbKind` | ✅ |
| رسائل خطأ مصنّفة (مصادقة / مخطط / مهلة) بلا أسرار | ✅ |
| قراءة `DATABASE_URL` حيّة + عدم إعادة ترميز كلمة المرور | ✅ |
| اختبارات 194/194 + CI أخضر | ✅ |

**استكشاف الأخطاء بعد النشر:**
- `dbKind: "auth"` → كلمة مرور `DATABASE_URL` قديمة → انسخ الرابط (pooled) من Neon Console إلى Vercel ثم Redeploy.
- `dbKind: "schema"` → `cd app && npm run db:push && npm run admin:ensure`.
- `dbKind: "timeout"/"network"` → راجع حالة Neon (endpoint متوقف؟) وأعد المحاولة.
...
## المرحلة 32 — توأم شمسي/مروحة + a11y مخطط + حدود أخطاء (منفَّذة)

| البند | الحالة |
|--------|--------|
| SunRingPrint: شارة توأم للأزواج أيضاً | ✅ |
| FanChartView: علامة توأم مرئية على الأوتاد والجذر | ✅ |
| FamilyChart: TwinBadge على فلاتر الزوجات وفروع الأم | ✅ |
| شريط أدوات الشجرة + نقطة البحث + رابط الزوجية: aria-label | ✅ |
| formatYears ورسالة ErrorBoundary أوضح | ✅ |
| TwinBadge على الجد المشترك في ShareView | ✅ |
| اختبارات occasionShare | ✅ |
