...
export type ConsistencyTourScope = OccasionsScope;
...
const key = (treeId: number) => `nasab:consistencyTour:${treeId}`;

export function getConsistencyTourState(treeId: number): TourState {
  try {
...
export function setConsistencyTourState(
  treeId: number,
  state: TourState,
): TourState {
  try {
