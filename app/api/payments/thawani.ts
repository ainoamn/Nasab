...
export const thawaniAdapter: PaymentAdapter = {
  ...
  async createCheckout(config, input, isTestMode) {
    ...
    const successUrl = `${input.origin}/api/checkout/complete?invoice=${encodeURIComponent(input.invoiceNumber)}&gateway=thawani`;
  ...
  async verifyReturn(config, params) {
    ...
    const body = (await resp.json()) as {
      data?: { payment_status?: string; client_reference_id?: string };
